...
actually built (real code) today.

> **Status column refreshed 2026-08-21.** Written 2026-08-05, when Tenant,
> Site, Agent and Device were partition-key prefixes and configuration
> rather than rows. Phase 3's admin work made all four real tables, and the
> `ICapability` stub this file described was later deleted as unused. Six
> rows below were wrong about what is built; they now match the schema in
> `stvivnestagent2`, verified table by table. **The vocabulary was not
> touched** - the distinctions this file draws (Capability vs Bridge, Native
> vs Bridged, Health vs Metrics, Trigger vs rules engine) were correct then
> and remain correct, and are not written down anywhere else. Only the
> "Status today" column moved. Nothing here authorizes building a Site
...
|---|---|---|
| **Tenant** | **Real table** (`tblTenants`) plus the partition-key prefix (`PartitionKey = "{TenantId}\|{SiteId}"`). 10 `/tenants` routes. Was "no table, never a row" until Phase 3. | A customer account. Resolved per-request from an API key into `TenantContext` (`Vivnest.Cloud/Auth/TenantContext.cs`). Never a row. |
| **Site** | **Real table** (`tblSites`), reached through `/tenants/{tenantId}/sites`. [ADR-018](decision-log.md) rejected a Site entity *at the time*; Phase 3 revisited that and built one. The partition-key prefix still exists alongside it. | A physical location under a tenant. Same status as Tenant: a partition-key prefix and a field on `TenantContext`/heartbeats. Stays this way — confirmed still fine as-is, not hitting a real limitation yet. |
| **Agent** | **Real registry table** (`tblAgentRegistry`) with full CRUD, plus the heartbeat/event rows (`AgentHeartbeatEntity`, `AgentEventEntity`) and an installation record (`tblAgentInstallations`). No longer known only through the telemetry it emits. | One running `Vivnest.Agent` process. Known to the system only through the heartbeats/metrics it emits — no `Agents[]` config or CRUD row independent of a heartbeat having arrived. |
| **Device** | **Real registry table** (`tblDeviceRegistry`) with full CRUD, plus heartbeat/event rows and the published `DeviceOptions` document. Identity is now *issued* by the registry (`DeviceId`) and mapped to what the agent calls itself (`RuntimeDeviceId`) - the opposite of the arrangement described when this was written. | One physical sensor/actuator an agent talks to. Identity (`DeviceId`) is asserted in agent-local config, not issued by a cloud registry. One flat `DeviceOptions` shape for every type (no `CameraOptions`/etc. subtyping) — `Location`/`Brand`/`Model`/`Firmware` are plain descriptive fields on it, never read by any capability's worker. |
| **Capability** | Real: `Vivnest.Agent/Capabilities/{Type}/` folders, **plus a cloud-side registry** (`tblCapabilities`, `tblDeviceCapabilities`, `tblAgentCapabilities`, `tblCapabilityDependencies`, `tblDeviceTypeCapabilities`). The `ICapability` interface described here **was deleted** - it had zero implementations and zero references. Still **no Capability Host**: `tblAgentCapabilities` has admin CRUD and no runtime consumer, so assigning a capability to an agent changes nothing about what it does. | A distinct **ability** ("capture," "toggle power," "detect motion") plus whatever it takes to exercise that ability for one kind of device — connection, worker loop, state, events. See §3. |
...
| **Schedule** | Real, partial: `DeviceOptions.Schedule` (`ScheduleOptions`: `Interval`, `Burst`), with the loop living in `CameraCaptureWorker`. The empty `SnapshotScheduler.cs` stub referenced here has since been deleted. | `Interval`/`Burst` built; calendar recurrence (`Cron`/`Window`) is still just design. See §6. |
...
| **DeviceType** | Enum, now nine members (`Camera, HumiditySensor, SmokeAlarm, WaterLeak, HeatPump, MotionSensor, DoorSensor, SmartPlug, Hub`) — `Hub` added since. A `tblDeviceTypes` table also exists for admin metadata, which does not replace the enum: the enum still gates handler/UI dispatch. | Stays an enum — cheap to extend, already gates handler/UI dispatch correctly. Promote to a catalog table only if tenant-editable device-type metadata is ever needed. |
