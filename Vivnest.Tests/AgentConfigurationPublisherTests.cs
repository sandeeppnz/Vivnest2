...
public class AgentConfigurationPublisherTests
{
    ...
    // ---- capability assignments on the wire (ADR-096) -------------------

    // The whole point of the assignment path: what Cloud assigns has to
    // arrive at the Agent under the key the Agent's manifests actually use.
    // CapabilityId stays the catalogue GUID; CapabilityKey is the join.
    [Fact]
    public async Task AssignedCapabilitiesReachTheAgentConfigBlob()
    {
        var h = new Harness();
        ...
        var capabilities = JsonSerializer.Deserialize<JsonElement>(flat)
            .GetProperty("Capabilities");

        Assert.Equal(1, capabilities.GetArrayLength());
        Assert.Equal("camera.capture", capabilities[0].GetProperty("CapabilityKey").GetString());
    ...
    // the ADR-069 no-op guard would swallow an assignment change - the
    // config would look unchanged, no new version would be written, and
    // enabling or disabling a capability would appear to do nothing at all.
    [Fact]
    public async Task ChangingACapabilityAssignmentDefeatsTheNoOpGuard()
    {
        var h = new Harness();
        ...
        var first = await h.Publisher.PublishAsync(Tenant, AdminAgentId);
        Assert.True(first!.Published);

        // Same document in every respect except the Enabled flag.
        h.Projector.Document = h.Projector.Document with
        {
            Capabilities =
            [
