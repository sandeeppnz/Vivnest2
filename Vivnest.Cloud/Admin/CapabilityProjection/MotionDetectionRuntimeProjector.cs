        ...
        string? executingRuntimeAgentId)
    {
        ...
        if (assignedSettings == null)
        {
            return new CapabilityProjectionResult(
                null, null,
                ["Motion Detection: assignment Settings is not valid JSON; nothing published for it."]);
        }
        ...
        {
            if (double.TryParse(batteryReportInterval, NumberStyles.Float, CultureInfo.InvariantCulture, out _))
            {
